<div>
    ...
    <div class="card">
      ...
      <div class="mat-card-header header-pink-gradient">
        <h5 id="cardHeaderTitleAllsections">Exam Hall</h5>
        <p id="cardHeaderSubtitle">Manage exam halls, create, update, and organize academic halls easily.</p>
      </div>
        ...
        <div class="card-header border-0">
            ...
            <div class="card-toolbar">
                ...
                <button class="btn-outline bg-dark text-white" wire:click="openCreate">
                    <span class="material-icons-round">add</span> <span id="newSectionBtn">Add Hall</span>
                </button>
        ...
        <div class="card-body pt-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($halls as $i => $hall)
                        <tr>
                            <td class="text-muted">{{ $halls->firstItem() + $i }}</td>
                            <td> {{ $hall->hall_no }} </td>
                            <td> {{ $hall->no_of_seat }} </td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="act-btn edit" title="Edit" wire:click="openEdit({{ $hall->id }})">
                                        <span class="material-icons-round">drive_file_rename_outline</span>
                                    </button>
                                    <button class="act-btn delete" title="Delete" wire:click="confirmDeleteRecord({{ $hall->id }})">
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-5 text-muted">
                                ...
                                No halls found. <a href="#" wire:click.prevent="openCreate">Create one now</a>.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="card-footer border-0 bg-white d-flex align-items-center justify-content-between flex-wrap gap-2 py-2 px-3">
            <small class="text-muted">Showing {{ $halls->firstItem() ?? 0 }}–{{ $halls->lastItem() ?? 0 }} of {{ $halls->total() }}</small>
           {{ $halls->links('vendor.pagination.custom') }}
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form wire:submit.prevent="save">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    <label class="form-label">Hall No <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control @error('hall_no') is-invalid @enderror" wire:model.defer="hall_no" placeholder="e.g. Hall One">
                                    @error('hall_no') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label">No of Seats <span class="text-danger">*</span></label>
                                    <input type="number" class="form-control @error('no_of_seat') is-invalid @enderror" wire:model.defer="no_of_seat" placeholder="e.g. 100">
                                    @error('no_of_seat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <div class="modal-dialog modal-sm">
                <div class="modal-content">
                    <div class="modal-body text-center py-4">
                        ...
                        <h6 class="fw-700">Delete Hall?</h6>
