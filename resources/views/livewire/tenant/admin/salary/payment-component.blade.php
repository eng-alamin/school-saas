            ...
            <table class="table-loader">
                ...
                <tbody>
                    ...
                    <tr wire:key="emp-{{ $employee->id }}">
                        ...
                        <td>
                            @if(($employee->salary_status ?? '') === 'paid')
                                <a class="action-btn btn-payslip" href="{{route('admin.salary.invoice-payment', ['tenant' => tenant('id'), 'id' => $employee->id, 'month' => $this->month]) }}" target="_blank">
                                    <span class="material-icons-round" style="font-size:14px;vertical-align:middle">visibility</span>
                                    Payslip
                        </a>
                            @else
                                <a class="action-btn btn-paynow" href="{{route('admin.salary.add-payment', ['tenant' => tenant('id'), 'id' => $employee->id, 'month' => $this->month]) }}" target="_blank">
