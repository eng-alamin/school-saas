<div class="mat-card" style="padding-top:28px">
      ...
      <div class="table-card">
        ...
        <div class="card-toolbar">
          ...
          <!-- buttons right -->
          <a href="{{ route('tenant.academic.classes') }}" class="btn-outline bg-dark text-white">
            <span class="material-icons-round" style="font-size:16px">flight_class</span><span>Class</span>
          </a>
          <a href="{{ route('tenant.academic.sections') }}" class="btn-outline">
          ...
          <button class="btn-outline" data-bs-toggle="modal" data-bs-target="#openCreateModal">
            <span class="material-icons-round">add</span> <span id="newClassBtn">New Class</span>
          </button>
          ...
          <table class="mat-table" id="productsTable">
            <thead>
              <tr>
                ...
                <th onclick="sortTable(2)" id="th-numeric"><span id="th-numeric-lbl">Numeric</span> <span class="sort-icon"></span></th>
                <th onclick="sortTable(3)" id="th-section"><span id="th-section-lbl">Section</span> <span class="sort-icon"></span></th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body p-4" style="display:flex;flex-direction:column;gap:14px">
                    ...
                    <div>
                        <label style="font-size:.75rem;font-weight:600;color:var(--dark);display:block;margin-bottom:4px">Numeric *</label>
                        <input wire:model="numeric" type="text" placeholder="e.g. 1" style="width:100%;border:1px solid rgba(0,0,0,.12);border-radius:8px;padding:8px 12px;font-size:.82rem;font-family:inherit;outline:none"/>
                        @error('numeric') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer border-0">
                    <button class="btn-outline" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" wire:click="update({{ $class_id }})" class="btn-pink">Update</button>
                </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            ...
            <h6 style="font-weight:700;margin:8px 0 4px">Delete this class?</h6>
    ...
    <script>
        ...
        function getVal(p, col) {
            switch(col) {
                ...
                case 2: return Number(p.numeric) || 0;
                case 3: return p.section ? p.section.name : '';
                default: return '';
            }
        }
        ...
        function renderTable() {
            ...
            tbody.innerHTML = rows.map((p, index) => `
                <tr id="row-${p.id}">
                    ...
                    <td data-label="Numeric"><span>${p.numeric ?? p.numeric ?? ''}</span></td>
                    <td data-label="Section"><span>${p.section ? p.section.name : 'N/A'}</span></td>
        ...
        document.getElementById('tableSearch').addEventListener('input', function() {
            ...
            filteredData = classes.filter(p =>
                String(p.id ?? '').toLowerCase().includes(q) ||
                String(p.name ?? p.name ?? '').toLowerCase().includes(q) ||
                String(p.numeric ?? p.numeric ?? '').toLowerCase().includes(q) ||
                String(p.section ? p.section.name : '').toLowerCase().includes(q)
            );
            currentPage = 1;
            renderTable();
        });
        ...
        function openDeleteModal(id) {
            deleteTargetId = id;
            const p = classes.find(x => x.id === id);
            if (!p) return;
        ...
        function confirmDelete() {
            ...
            const idx = classes.findIndex(x => x.id === deleteTargetId);
            if (idx > -1) {
        ...
        document.addEventListener('livewire:initialized', () => {
            Livewire.on('refresh-list', (data) => {
                ...
                classes.length = 0;
                newData.forEach(d => classes.push(d));
                filteredData = [...classes];
                renderTable();
